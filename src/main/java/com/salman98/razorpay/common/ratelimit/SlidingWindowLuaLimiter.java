...
public class SlidingWindowLuaLimiter implements RateLimiter {
    ...
    // Purpose of Lua Script - Make thread safe rate limiting
    // Because redis executes the Lua script atomically, so another request can't jump into the middle.
    ...
    public RateLimitResult check(String key, int maxRequests, long windowSeconds) {
        try {
            ...
            // 1. providing all values required for the lua script
            ...
            // 2. getting values in form of list
            boolean allowed = result.get(0) == 1L;
            int remaining = result.get(1).intValue();
            long oldestScoreMs = result.get(2);

            // 3. find retry after time
            if (!allowed) {
